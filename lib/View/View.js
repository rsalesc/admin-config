...
class View {
    ...
    _validateRecursively(entry, fields){
        fields.map((field) => {
            if(field.type() === 'embedded_list')
                this._validateRecursively(entry, field.targetFields());
    ...
    validate(entry) {
        this._validateRecursively(entry, this._fields);
    }
